...
impl Device {
    ...
    pub fn new(data: &Value) -> Option<Self> {
        let is_device =
            data.get("type").and_then(Value::as_str) == Some("PipeWire:Interface:Device");

        let media_class = data
            .get("info")?
            .get("props")?
            .get("media.class")
        ...
        let routes: Vec<EnumRoute> = params
            ...
            .map(|arr| arr.iter().filter_map(|obj| EnumRoute::new(obj)).collect())
            .unwrap_or_default();

        let profiles: Vec<EnumProfile> = params
            .and_then(|p| p.get("EnumProfile"))
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter().filter_map(|obj| EnumProfile::new(obj)).collect())
            .unwrap_or_default();
        ...
        let current_profile =
            current_profile_id.and_then(|id| profiles.iter().find(|p| p.index == id).cloned());
